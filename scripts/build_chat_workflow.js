...
- tool="revenue", date_list_csv="YYYY-MM-DD,YYYY-MM-DD,...": per-day, per-BU revenue + units_sold + pax for a SHORT list of explicit dates (up to ~31). Batch all dates of ONE period into ONE call ("this week vs last week" = 2 calls, one per week, so each summary is that period's). For anything longer than a month, use revenue_range instead. NOTE: units_sold = product units (NOT covers); pax = real covers/tickets/orders.
- tool="revenue_range", start_date="YYYY-MM-DD", end_date="YYYY-MM-DD", bu_names_csv="" (optional filter): server-side period analytics for ANY range — returns summary with total_revenue, total_pax, by_bu[] (revenue+pax), by_month[], by_weekday[] (revenue, days, avg_per_day), best_day, worst_day, days_with_sales. THE tool for whole months/years, weekday patterns ("which weekday sells most"), best/worst day in a period, and monthly breakdowns. One call per period being compared.
...
- tool="product_daily", q="<term>", start_date="YYYY-MM-DD", end_date="YYYY-MM-DD": DAY-BY-DAY sales of the products matching the term — one row per (date × product × BU) with uds and net revenue, plus a pre-computed summary (total_uds, total_revenue, days_with_sales, by_bu). Use it for ANY daily/per-date breakdown of a product ("desglosa por día", "qué días se vendió", "detalle del 8 de julio"). Use a specific term and a bounded range (max 1000 rows; summary.truncated tells you if capped).
- tool="open_days", start_date="YYYY-MM-DD", end_date="YYYY-MM-DD": days OPEN vs CLOSED for EVERY BU in the range, pre-computed server-side. Returns summary.by_bu[] with days_open_total, by_year, by_month[] ({month, open, days_in_range}) and by_weekday_open (mon..sun counts). Use it for ANY question about open/closed/operational day COUNTS, opening patterns or weekly schedules — ONE call covers all BUs and the whole range. IMPORTANT: it returns COUNTS ONLY, never specific dates. If the user asks WHICH exact dates a BU was open/closed, additionally call tool=revenue with every date of that month in one date_list_csv and read rows[] filtered to that BU: dates with no row (or revenue<=0 and pax<=0) are the closed ones. NEVER deduce a list of specific dates from counts or weekday patterns — that fabricates dates.
- tool="list": no args, returns canonical BU names.

RESPONSE RULES:
1. NUMBERS: every data tool returns a pre-computed "summary" object — use it for EVERY total, subtotal and per-BU/per-month/per-weekday figure (summary.total_revenue, summary.by_bu[], summary.by_day[], summary.by_month[], summary.by_weekday[], summary.total_uds...). NEVER add up the raw rows[] yourself — adding many numbers by hand produces wrong totals. rows[] is only for row-level detail the summary does not already give. Your headline number MUST be the summary total, never your own sum of a table you built.
...
function buildWorkflow() {
    const nodes = [
        ...
        // 5. Single tool: dispatcher subworkflow
        {
            parameters: {
                name: 'sales_query',
                description: 'Consulta la base de datos de ventas. Pasa "tool" + los args necesarios:\n- tool="search", q="<producto>": busca productos por nombre (LIMONADA, CERVEZA, ...). Acepta start_date/end_date opcionales.\n- tool="product_daily", q="<producto>", start_date, end_date: ventas DIA A DIA de los productos que casan con el termino (fila por fecha x producto x BU + summary). Para desgloses diarios.\n- tool="transactions", year_arg=2024, bu_names_csv="Juntos house,Tasting place": transacciones por BU y mes. bu_names_csv vacio = todas las BU.\n- tool="revenue", date_list_csv="2026-05-12,2026-05-05": revenue/units/pax por BU para pocas fechas concretas (max ~31).\n- tool="revenue_range", start_date, end_date, bu_names_csv opcional: analitica de periodo servidor-side (total, por BU, por mes, por dia de semana, mejor/peor dia, pax). Para meses/años/patrones semanales.\n- tool="open_days", start_date="2025-01-01", end_date="2026-07-09": dias abiertos/cerrados por BU (totales, por año, por mes, por dia de semana) pre-calculados.\n- tool="list": lista nombres canonicos de BU.\nUsa "Picadeli" si el usuario dice "Juntos deli".',
                workflowId: { __rl: true, value: DISPATCHER_ID, mode: 'list', cachedResultName: DISPATCHER_NAME },
                workflowInputs: {
                    mappingMode: 'defineBelow',
                    value: {
                        tool: "={{ $fromAI('tool', 'Acción: search, transactions, revenue, revenue_range, top_products, product_daily, open_days o list', 'string') }}",
                        q: "={{ $fromAI('q', 'Para tool=search o product_daily: termino del producto. Vacio para otras tools.', 'string', '') }}",
                        year_arg: "={{ $fromAI('year_arg', 'Para tool=transactions: año entero (2024, 2025). 0 para otras tools.', 'number', 0) }}",
                        bu_names_csv: "={{ $fromAI('bu_names_csv', 'Para tool=transactions o revenue_range: BU separadas por coma. Vacio para todas.', 'string', '') }}",
                        ...
                        start_date: "={{ $fromAI('start_date', 'Para tool=top_products, open_days, search, product_daily o revenue_range: fecha inicio YYYY-MM-DD (incluida). Vacio = todo el historico.', 'string', '') }}",
                        end_date: "={{ $fromAI('end_date', 'Para tool=top_products, open_days, search, product_daily o revenue_range: fecha fin YYYY-MM-DD (incluida). Vacio = hasta hoy.', 'string', '') }}",
