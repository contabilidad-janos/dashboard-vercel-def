...
const CanEscarrerFilters = ({
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm mb-6">
            ...
            {/* Row 3b: Tipo cliente chips (Restaurante / Tienda / Cliente directo) */}
            {tiposCliente.length > 0 && (
                <div className="px-5 py-3 border-b border-gray-100">
                    <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Tipo cliente</p>
                    <div className="flex flex-wrap gap-2">
                        <button
                            onClick={() => toggleTipo('__ALL__')}
                            className={clsx('bu-chip font-semibold text-xs', selectedTipos.length === 0 && 'active')}
                        >All</button>
                        {tiposCliente.map(t => (
                            <button key={t} onClick={() => toggleTipo(t)} className={clsx('bu-chip text-xs', selectedTipos.includes(t) && 'active')}>{t}</button>
                        ))}
                    </div>
                </div>
            )}

            {/* Row 3c: Client exclusion (internal consumption) */}
                ...
                <div className="px-5 py-3 border-b border-gray-100">
                    <div className="flex items-center justify-between mb-2 gap-3 flex-wrap">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            >
                                <UserX className="w-3 h-3" /> Excluir internos (ORIGEN=INTERNO)
                            </button>
                            {excludedClients.length > 0 && (
                                <button
                        ...
                        {clientOptions.length > 12 && (
                            ...
                            >
                                {showAllClients
                                    ...
                                    : <>+{clientOptions.length - 12} más <ChevronDown className="w-3 h-3" /></>}
                            </button>
                        )}
                    </div>
                </div>
            ...
            {/* Row 4: Marca + Origen + search */}
            <div className="flex flex-wrap items-center gap-3 px-5 py-3 bg-gray-50/50">
                <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Marca</span>
                <select
                ...
                {origenValues.length > 0 && (
                    <>
                        <div className="w-px h-5 bg-gray-200" />
                        <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Origen</span>
                        <select
                            value={selectedOrigen || ''}
                            onChange={e => setSelectedOrigen(e.target.value || null)}
                            className="border border-gray-200 rounded-lg px-3 py-1.5 text-sm text-gray-700 focus:ring-1 focus:ring-accent focus:outline-none bg-white"
                        >
                            <option value="">All</option>
                            {origenValues.map(o => (<option key={o} value={o}>{o}</option>))}
                        </select>
                    </>
                )}
...
export default CanEscarrerFilters;
