...
const Dashboard2025 = () => {
    ...
    const [salesData24, setSalesData24] = useState(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const sales24 = await DataService.get2024SalesData();
    ...
    function updateView(unit) {
        ...
        if (unit === 'all') {
            ...
            if (salesData24) {
                currentSales24 = MONTHS.map((_, i) => businessUnits.reduce((sum, b) => sum + getDisplayValue((salesData24[b.name][i] || 0), b.name), 0));
            }
            ...
            currentSales24 = salesData24 ? (salesData24[unit] || []).map(v => getDisplayValue(v, unit)) : [];
            currentBudget = (budgetData[unit] || []).map(v => getDisplayValue(v, unit));
    ...
    }, [selectedUnit, salesData, salesData24, businessUnits, budgetData, includeVat]);
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <KPICard title="Total Sales 2025" value={formatCurrency(kpis.totalSales)} subtext={selectedUnit === 'all' ? 'Total Group' : displayBuName(selectedUnit)} />
