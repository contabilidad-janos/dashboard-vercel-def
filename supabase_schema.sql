...
-- 5. Can Escarrer Sales Table
-- Line-level invoice data for the Can Escarrer side (DISTRIBUCION / SHOP / TASTING / SERVICES).
-- Source: Google Sheets export "VENTAS CAN ESCARRER - VENTAS F 2024.csv".
-- No hour granularity. `bu` distinguishes the sub-units.
create table public.can_escarrer_sales (
  id uuid default uuid_generate_v4() primary key,
  date date not null,
  ...
  familia text,
  marca text,
  budget text,                     -- Budget category (FRESH PRODUCTS, etc.)
  uds numeric(10,3) default 0,
  importe numeric(12,2) default 0,
...
create policy "Enable read access for all users" on public.can_escarrer_sales for select using (true);
